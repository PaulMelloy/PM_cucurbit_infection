...
br_tm2 <- fread("cache/Tm_branches.csv")
```
...
g_tm[Tm != 8 &
   ...
   labs(colour = "Time (h)")+
   theme_classic()+
   scale_x_continuous(breaks = c(18,21,24,27,30))+
   scale_y_continuous(limits = c(0,100), n.breaks = 5)
```
...
```{r model_2}
m2 <- gam(cbind(germ_conidia, non_germ_conidia) ~ te(i_period, Tm, k = 5),
          data = g_tm,
          family = "binomial")
summary(m2)
plot(m2)
...
While the anova says `m3` is a significantly better model, the residual deviance 
informs us it is not by much. 
Because the plot of `m4` seems a more reasonable fit to the data, we will use this.  
...
coef(m4)
```
There are five cooeficients for the Temperature variable because the b-spline breaks
the data into different ranges depending on `knot` placement. 
Here we see there are four knots
...
range, and therefore the only inference we can make from these values is some are 
greater some are less and some have a large negative influence on spore germination
state.  

So for temperatures < 8$^\circ$C the coefficient was `r coef(m4)[1]`, with a low t-value
indicating little difference between the number of conidial germ-tubes at temperature
treatments in this range.
For temperatures 8$^\circ$C < `Tm` < 23$^\circ$C the coefficient was `r coef(m4)[2]`, 
with a t-value greater than 2 indicating a likely significant difference between the
...
with a t-value less than -2 indicating a likely significant difference and negative 
association between the proportions of conidia with varying numbers of germ-tubes 
due to temperatures in this range.


If we take the exponent of the coefficients, we will obtain the *odd ratio* which 
...
round(
   ...
   3)
```
The odds ratio still does not make much sense, given that an odds ratio of 1, means 
no change and an odds ratio of 2 is twice as likely.
An odds ratio of 11.6 indicates 11.6/1 times more likely with each degree increase
a spore will develop another germ-tube. 
This seems excessive, because it is.
We still need to back-transform the splines, prior to taking the exponent.  
