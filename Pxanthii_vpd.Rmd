...
```{r}
m4v <- gam(cbind(germ_n,non_germ_n) ~ s(vpd, k = 7, bs = "ps") + s(Tm, k = 3) + i_period, 
           data = g_vpd,
           family = "binomial")
```
...
b_v |>
   ...
   geom_text(aes(x = as.factor(vpd), y = 115, label = Tm),size = 3,)+
...
m2 <- polr(branches ~ bs(vpd) + Tm+exp, data = b_vpd_lng, Hess = TRUE)
unique(b_vpd[,RH])
summary(m2)
summary(m3)
anova(m2,m3)
...
There is a consistant effect of temperature that increases the number of germinated
