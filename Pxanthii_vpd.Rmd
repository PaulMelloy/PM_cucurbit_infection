...
Figure_4 <- 
   ...
   geom_smooth(method = "gam",formula = y ~ bs(x,k=0.2), alpha = 0.3, size = 0.8)+
   scale_color_viridis_d()+
...
The splines on `Tm` and `i_period` were statistically significant, and seem well 
fit and logical when we refer them to the raw data. 
...
While there exists a lot of willingness in the gam, the vpd plot shows the increase in germination in at around 1.5 kPa and the rapid decrease in germination between the saturated treatments (0.026 - 0.038 kPa) and the next highest humidity (0.158 - 0.189 kpa).
...
Temperature does not seem like it needs a non-linear terms and test to see if there
is any significant changes.  
...
anova(m4v,m5v)
```
This comparison shows no interaction between temperature and VPD reduces the fit 
of the model.  

Because of the rapid drop in germination between the lower VPD range, and little change 
...
knots without causing an overfit model. 
...
Let's compare the model against the others.  
...
b_vpd[, list(n = mean(conidia)),
      by = list(vpd, Tm, germtubes)][order(vpd,Tm)]
...
We will combine the data into one object to interegate.  
```{r bv_combine}
