...
library(MASS) # Logistical ordinal regression
```
<br>  
...
```{r}
b_vpd[, list(n = mean(n),
             non_germ = mean(non_germ)),
      by = list(vpd, Tm, RH, germ_tubes)][order(vpd,Tm)]

b_tm_vpd[ ,list(n = mean(conidia)),
      by = list(vpd, Tm, branches)][order(vpd,Tm)]

b_tm_vpd2[ ,list(n = mean(conidia)),
      by = list(vpd, Tm, RH,branches)][order(vpd,Tm)]
...
b_vpd |>
   ...
   theme_classic()+
   geom_label(aes(x = as.factor(vpd), y = 80, label = Tm))+
...
m1 <- polr(germ_tubes ~ vpd + Tm, data = b_vpd_lng, Hess = TRUE)
