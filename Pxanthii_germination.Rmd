...
```{r}
m1v <- gam(cbind(germ_n,non_germ_n) ~ s(vpd, k = 6) + s(Tm, k = 3) + s(i_period, k = 3), 
           data = g_vpd,
           family = "binomial")
summary(m1v)
plot(m1v)
...
```{r}
m2v <- gam(cbind(germ_n,non_germ_n) ~ s(vpd, k = 7, bs = "ps") + s(Tm, k = 3) + s(i_period, k = 3), 
           data = g_vpd,
           family = "binomial")
summary(m2v)
plot(m2v)
