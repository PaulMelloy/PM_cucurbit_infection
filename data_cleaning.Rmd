...
br_tm <- fread("cache/Tm_branches.csv", 
              col.names = c("obs", "Tm", "branchs","rep", "count"))

# Add relative humidity and temperature data
br_tm[ , c("Tm","branchs", "obs") := list(as.numeric(gsub("oC","",Tm)),
                                         as.numeric(gsub("b","",branchs)),
                                         NULL)]

# recalculate vpd and convert branches to numeric
zero_branch <- br_tm[, (100 - sum(count)), 
                      by = .(Tm, rep)][,branchs :=0][, .(Tm, branchs, rep, V1)]

setnames(zero_branch,old = "V1", new = "count")

# Bind in the data containing non-germinated conidia as conidia with zero branches
br_tm <- rbind(br_tm,zero_branch)[order(Tm,branchs, rep)]

fwrite(br_tm, "cache/Tm_branching.csv")
```
