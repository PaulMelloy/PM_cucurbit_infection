...
germ_tm <- rbindlist(
   list(
      ...
      list(Tm = c(rep(8, 4 * 4),
                  rep(35, 4 * 4)),
      i_period = rep(c(12, 24, 26, 48), each = 4*2),
      leaf = rep(1:2, each = 2, times = 2 * 2 * 2),
...
br_tm <- rbind(br_tm,zero_branch)[, conidia := round(conidia)][order(Tm,branches, rep),]
```
...
```{r save_br_tm}
fwrite(br_tm, "cache/branching_temperature.csv")
```
